...
from jre.dhse_contract import CONTRACT_VERSION, schema_summary  # noqa: E402
...
def _methods_snapshot(summary: Dict[str, Any], validation: Dict[str, Any]) -> str:
    ...
    field_counts = validation.get("field_roles", {}).get("counts", {})
    return "\n".join(
        [
            ...
            "",
            f"Data contract: `{CONTRACT_VERSION}`.",
            "",
            "## Input Contract",
            "",
            ...
            f"- Snapshot fields present: {field_counts.get('snapshot_scoring_input')}",
            f"- Label fields present: {field_counts.get('post_disposition_label_only')}",
            f"- Leakage-risk columns: {len(validation.get('leakage_risk_columns', []))}",
